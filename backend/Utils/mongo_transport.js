...
const { SERVICE_NAME, LOG_DB_URI, LOG_DB_NAME, LOG_BATCH_SIZE, LOG_INTERVAL, LOG_COLLECTION_NAME } = require("../ENV/env");
const { get_current_date_time, get_current_date, get_current_time } = require("./time");
const { LOG_LEVEL } = require("../Consts/logger_constants");
...
class DBTransport extends TransportStream {
  ...
  async initDB() {
    try {
      ...
      console.error(
        `${get_current_date_time()} | ERROR | Error in Connecting to Mongodb : `,
        err
      );
    }
  }
  ...
  async empty_log_list() {
    ...
    if (this.logs.length > 0) {
      try {
        ...
        console.error(
          `${get_current_date_time()} | ERROR | Error in Adding Log to Database : `,
          err
        );
      }
    }
  ...
  async log(info, callback) {
    try {            
      if (info.level !== "notify" && info.level !== "notifylog") {
        let object_to_log = {
          level: info.level,
          level_no: LOG_LEVEL["db"] - LOG_LEVEL[info.level],
          date: get_current_date(),
          time: get_current_time(),
          timestamp: Date.now(),
          timze_zone: "IST (UTC+5:30)",
      ...
      console.error(
        `${get_current_date_time()} | ERROR | Error in pushing to log queue : `,
        err
      );
    }
    this.send_to_db();
  ...
  async closeConnection() {
    if (this.client) {
      try {
        ...
        console.log(
          `${get_current_date_time()} | INFO | LOG DATABASE Connection Closed`
        );
      } catch (err) {
        // await send_notification("ERROR WHILE CLOSING CONNECTION");
        console.error(
          `${get_current_date_time()} | ERROR | Error in Closing MongoDB Connection: `,
          err
        );
      }
    }
